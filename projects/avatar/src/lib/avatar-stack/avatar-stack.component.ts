...
import { coerceNumberProperty } from '@laurelspa/ui/core';
import { LsAvatarComponent } from '../avatar.component';

@Component({
  selector: `ls-avatar-stack, div[ls-avatar-stack]`,
  exportAs: 'lsAvatarStack',
  templateUrl: './avatar-stack.component.html',
  styleUrls: ['./avatar-stack.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LsAvatarStackComponent implements AfterContentInit {
  ...
  @ContentChildren(LsAvatarComponent)
  public avatars: QueryList<LsAvatarComponent>;
  ...
  public constructor(
    ...
    elementRef.nativeElement.classList.add('ls-avatar-stack');
  }
  ...
  public ngAfterContentInit(): void {
    if (this.max) {
      ...
      setTimeout(() =>
        this.avatars.forEach((avatar: LsAvatarComponent, index: number) => {
          if (index >= this.max) {
            avatar.hidden = true;
          }
        })
